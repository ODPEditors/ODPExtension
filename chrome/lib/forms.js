(function() {
	...
	//	hbox.setAttribute('onclick', 'this.nextSibling.hidden=!this.nextSibling.hidden;this.nextSibling.nextSibling.hidden=!this.nextSibling.nextSibling.hidden;');
		...
		container.openPopup(null, '', ((this.screenGetDocumentW(aDoc) - container.boxObject.width) / 2) - 3, ((this.screenGetDocumentH(aDoc) - container.boxObject.height) / 2), false, false);
		container.addEventListener('popuphidden', function(event) {
			ODPExtension.removeElement(event.currentTarget);
		}, false);
	}
