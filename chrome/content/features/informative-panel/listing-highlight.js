(function() {
	...
	this.listingsHighlight = function() {
		...
		//look at all the link in the page
			...
			this.foreachFrame(this.windowGetFromTab(this.tabGetFocused()), function(aDoc) {
				ODPExtension.listingsHighlightDoc(aDoc);
			})
		}
	}
	//looking a the doc
	...
	this.listingsHighlightItem = function(item) {
		...
		this.readURL(
			...
			null,
			null, function() {
			ODPExtension.listingsHighlightRetreiveInformationForItem(arguments[0], arguments[1], arguments[2]);
		}, item, tooltiptext); //variable num of arguments
	}
	...
	this.listingsHighlightRetreiveInformationForItem = function(aData, item, tooltiptext) {
		...
		var responseURL = '',
			responseCategory = '',
			results = 0,
			itemSubdomain = this.removeWWW(this.getSubdomainFromURL(item.href)),
			itemDomain = this.getDomainFromURL(item.href);
