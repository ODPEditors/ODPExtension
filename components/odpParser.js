...
ODPParser.prototype = {
	...
	htmlSpecialCharsDecode: function(aString) {
		return aString.split('&lt;').join('<').split('&gt;').join('>').split('&quot;').join('"').split('&apos;').join("'").split('&amp;').join('&');
	},
	stripTags: function(aString) {
		return aString.replace(/<[^>]*>/g, '')
	},
	categorySanitize: function(aCategory) {
		return this.htmlSpecialCharsDecode(aCategory.replace(/^[^\:]+\:/, '').replace(/^Top\//, '').replace(/\/+$/, '')) + '/';
	},
	categoriesIDs: [],
	categoryCount: 0,
	categoryID: function(aCategory) //resolves the category ID
	{
	...
	QueryInterface: function(aIID) {
		if (!aIID.equals(nsIODPParser) && !aIID.equals(nsISupports) && !aIID.equals(nsIObserver))
