(function() {
	...
	this.addListener('databaseReady', function() {
		...
		if (db.exists){
			//sql query
			query = db.query(' SELECT u.uri, u.description, u.title, c.category FROM uris u, hosts h, categories c where h.host = :host and h.id = u.domain_id and c.id = u.category_id order by h.host asc, u.uri asc');
		}
	});
