(function() {
	...
	this.addListener('databaseReady', function() {
		...
		if (db.exists){
			//sql query
			query = db.query(' SELECT u.uri, u.description, u.title, c.category, u.mediadate, u.cool, u.rss, u.atom, u.pdf FROM uris u, categories c where u.category_id in (select id from categories where category GLOB :category) and u.cool = 1 and c.id = u.category_id order by c.category asc, u.uri asc');
		}
	});
